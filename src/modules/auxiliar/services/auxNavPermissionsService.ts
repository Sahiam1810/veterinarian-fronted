...
import { AUX_ALWAYS_VISIBLE_NAV, AUX_MODULE_TO_NAV, fetchMyModulePermissions, filterNavKeysByModuleView } from '@/modules/auth'
import { AUX_DEFAULT_PERMISSIONS, AUX_NAV_PERMISSION_KEYS } from '@/global/navigation'
...
export async function fetchAuxNavPermissions(): Promise<GrantedPermissions> {
  try {
    ...
    return AUX_ALWAYS_VISIBLE_NAV.filter((key) => AUX_DEFAULT_PERMISSIONS.includes(key))
  }
}
