...
// Mapea cliente API a dueño UI (nombre, correo y estado vienen del cliente)
export function mapClientToDueno(
  client: ApiClientResponse,
  mascotasSummary: string[] = []
): SuperAdminDueno {
  const status: EstadoMascota = client.isActive === false ? 'Inactivo' : 'Activo'
  return {
    id: client.id,
    ...
    city: '',
    status,
    registrationDate: formatDateEs(client.createdAt),
    mascotasSummary,
  }
}
