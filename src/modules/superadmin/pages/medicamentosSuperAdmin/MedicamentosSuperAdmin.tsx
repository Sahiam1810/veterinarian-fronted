...
export function MedicamentosSuperAdmin({
  canCreate = true,
  canEdit = true,
  canDelete = true,
}: MedicamentosSuperAdminProps) {
  ...
  // Delete modal
  const [deletingMedication, setDeletingMedication] = useState<ApiMedication | null>(null)

  const loadData = async () => {
    setIsLoading(true)
    try {
      const list = await fetchMedications(filterActiveOnly)
      setMedications(list)
    } catch {
      showToast('Error al cargar la lista de medicamentos.', 'danger')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleOpenEdit = (item: ApiMedication) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      showToast('Medicamento desactivado correctamente.')
      setDeletingMedication(null)
      await loadData()
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Error al desactivar el medicamento.'
      showToast(msg, 'danger')
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-border-tan rounded-2xl shadow-xs overflow-hidden">
        {isLoading ? (
          <div className="p-8 text-center text-sage text-sm">Cargando catálogo de medicamentos…</div>
        ) : filtered.length === 0 ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-charcoal/40 backdrop-blur-xs">
          <div className="bg-white border border-border-tan rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
            <h3 className="text-lg font-extrabold text-brand flex items-center gap-2">
              ...
              <span>{editingMedication ? 'Editar Medicamento' : 'Nuevo Medicamento'}</span>
            </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Ej. Amoxicilina + Ácido Clavulánico 250mg"
                  value={name}
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  className="rounded border-border-tan text-brand focus:ring-brand"
                />
                <label htmlFor="med-active-chk" className="text-xs font-bold text-charcoal cursor-pointer">
                  Medicamento Activo
                </label>
              </div>
      ...
      {deletingMedication && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-charcoal/40 backdrop-blur-xs">
          <div className="bg-white border border-border-tan rounded-2xl shadow-2xl w-full max-w-sm p-5 space-y-4">
            <h3 className="text-base font-extrabold text-danger">Desactivar Medicamento</h3>
            <p className="text-xs text-charcoal">
              ¿Estás seguro de desactivar el medicamento <span className="font-bold text-brand">{deletingMedication.name}</span>?
            </p>
