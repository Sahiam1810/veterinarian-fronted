...
export function HospitalizacionListaView({
  ...
  const loadStays = useCallback(async () => {
    ...
    try {
      ...
      const msg =
        err instanceof Error ? err.message : 'Error al consultar las estancias de hospitalización.'
      setLoadError(msg)
    } finally {
      setIsLoading(false)
    }
  ...
  const filteredStays = stays.filter((stay) => {
    ...
    if (!q) return true
    return (
      (stay.petName && stay.petName.toLowerCase().includes(q)) ||
      (stay.ownerName && stay.ownerName.toLowerCase().includes(q)) ||
      (stay.motivo && stay.motivo.toLowerCase().includes(q)) ||
      (stay.admittedByName && stay.admittedByName.toLowerCase().includes(q))
    )
  })
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-sage mt-1">
            Gestión de pacientes internados, seguimiento de evolución clínica y admisiones.
          </p>
        </div>
