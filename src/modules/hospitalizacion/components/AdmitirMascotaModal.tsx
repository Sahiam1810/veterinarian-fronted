          ...
          <form onSubmit={handleSubmit} className="flex-1 min-h-0 flex flex-col overflow-hidden">
            <div className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-6 space-y-4">
              {loadPetsError && (
                <div
                  role="alert"
                  className="p-3 rounded-xl bg-danger-soft border border-danger/30 text-danger text-xs sm:text-sm font-medium flex flex-col sm:flex-row sm:items-center justify-between gap-2"
                >
                  <span className="flex-1">{loadPetsError}</span>
                  <button
                    type="button"
                    onClick={() => void loadOptions()}
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-danger text-white text-xs font-bold hover:bg-danger/90 transition cursor-pointer self-start sm:self-auto shrink-0"
                  >
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-xs font-bold text-sage uppercase tracking-wider">
                    Mascota a Hospitalizar *
                  </label>
                  {isLoadingPets && (
                    <span className="inline-flex items-center gap-1.5 text-xs text-brand font-medium">
                      <span className="w-3 h-3 border-2 border-brand/30 border-t-brand rounded-full animate-spin inline-block" />
